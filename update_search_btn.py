...
for file_name in glob.glob('*.html'):
    ...
    # Regex to find the current search modal block (we match the small version we just added)
...
print(f'Updated search HTML to include search button in {updated} files')
